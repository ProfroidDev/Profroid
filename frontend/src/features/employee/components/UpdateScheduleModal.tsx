...
export default function UpdateScheduleModal({
  ...
  function validate(): string | null {
    ...
    for (const day of DAYS) {
      if (!isTechnician) {
        ...
        if (startMinutes >= endMinutes)
          return t('error.schedule.startBeforeEnd', {
            day: t(`common.dayOfWeek.${day.toLowerCase()}`),
          });
        const dailyHours = (endMinutes - startMinutes) / 60;
        if (dailyHours > 8)
          return t('error.schedule.nonTechnicianMaxHoursPerDay', {
            day: t(`common.dayOfWeek.${day.toLowerCase()}`),
    ...
    if (weeklyHours > 40)
      return t('error.schedule.employeeMaxHoursPerWeek', { hours: weeklyHours.toFixed(1) });
    return null;
  }
  ...
  async function submit() {
    ...
    try {
      ...
      const weeklyConflict = message?.match(
        /Cannot update schedule: Employee has an existing appointment on (MONDAY|TUESDAY|WEDNESDAY|THURSDAY|FRIDAY) at (9:00 AM|11:00 AM|1:00 PM|3:00 PM|5:00 PM) that would be removed by this schedule update/i
      );
      if (weeklyConflict) {
      ...
      <div className="add-schedule-modal">
                    ...
                    .map((slot, idx) => {
                      ...
                      return (
                        <div className="slot" key={`${day}-${slot}-${idx}`}>
                          ...
                          <button
                            className="remove"
                            onClick={() => removeTechSlot(day, originalIdx)}
                            type="button"
                          >
                            {t('pages.employees.remove')}
                          </button>
                        </div>
                      );
                    })}
                  <div className="slot-add-controls">
                    {getAvailableSlots(isTechnician)
                      .filter((slotType) => !techSlots[day].includes(slotType))
                      .map((slotType) => (
                        <button
                          key={slotType}
        ...
        <div className="footer">
          ...
          <button className="primary" onClick={submit} disabled={submitting}>
            {t('pages.employees.updateSchedule')}
          </button>
        </div>
      </div>
    </div>
