...
export function getCategoryLabel(translate: (key: string) => string, value?: string): string {
  if (!value) {
    return '';
  }
...
export function getStatusLabel(translate: (key: string) => string, value?: string): string {
  if (!value) {
    return '';
  }
