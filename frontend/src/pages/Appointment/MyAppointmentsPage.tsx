    ...
    try {
      ...
      if (error instanceof Error && 'response' in error) {
        ...
        if (axiosError.response?.status === 404) {
          setError(
            axiosError.response?.data?.message ||
              "Customer not found or you don't have permission to view these appointments"
          );
          setAppointments([]);
          return;
        }
        if (axiosError.response?.status === 403) {
          setError(
            "You don't have permission to view appointments. Please ensure you're logged in as a customer."
          );
          setAppointments([]);
          return;
        }
              ...
              <div key={appointment.appointmentId} className="appointment-card">
                ...
                <div className="appointment-actions">
                  {appointment.status === 'SCHEDULED' &&
                    (appointment.createdByRole === 'CUSTOMER' ||
                      appointment.createdByRole === null ||
                      appointment.createdByRole === undefined) && (
                      <button
                        className="btn-edit"
            ...
            <div className="modal-content-light">
              <div className="detail-section">
                ...
                <p>
                  ...
                  <span
                    className={`modal-status-badge ${getStatusBadge(selectedAppointment.status)}`}
                  >
              ...
              <div className="detail-section">
                ...
                {selectedAppointment.technicianRole && (
                  <p>
                    <strong>{t('pages.appointments.role')}:</strong>{' '}
                    {typeof selectedAppointment.technicianRole === 'string'
                      ? selectedAppointment.technicianRole
                      : 'TECHNICIAN'}
                  </p>
                )}
                {technicianDetails?.phoneNumbers?.length ? (
                  <div>
                    {technicianDetails.phoneNumbers.map((ph, idx) => (
                      <p key={idx}>
                        <strong>{ph.type}:</strong> {ph.number}
                      </p>
                    ))}
                  </div>
                ) : null}
              ...
              <div className="detail-section">
                ...
                <p>
                  <strong>{t('pages.appointments.name')}:</strong> {selectedAppointment.cellarName}
                </p>
                {matchedCellar ? (
                  <>
                    <p>
                      <strong>{t('pages.appointments.type')}:</strong> {matchedCellar.cellarType}
                    </p>
                    <p>
                      <strong>{t('pages.appointments.dimensions')}:</strong> {cellarDimensions}
                    </p>
                    <p>
                      <strong>{t('pages.appointments.capacity')}:</strong>{' '}
                      {matchedCellar.bottleCapacity} bottles
                    </p>
                    <p>
                      <strong>{t('pages.appointments.features')}:</strong>{' '}
                      {[
              ...
              <div className="detail-section">
                ...
                <p>
                  ...
                  {selectedAppointment.appointmentAddress.province}
                </p>
                <p>
                  {selectedAppointment.appointmentAddress.country}{' '}
                  {selectedAppointment.appointmentAddress.postalCode}
                </p>
              </div>
              ...
              <div className="detail-section">
                ...
                <p>
                  <strong>{t('pages.appointments.customer')}:</strong>{' '}
