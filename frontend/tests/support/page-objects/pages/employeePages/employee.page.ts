...
export class EmployeePage {
  ...
  async rowByName(first: string, last: string) {
    return this.page.locator('tr', { hasText: new RegExp(`${last}.*${first}`, 'i') }).first();
  }
  ...
  // =====================================================
  deleteModal = () =>
    this.page.locator('.confirmation-modal-container').filter({ hasText: 'Delete Employee' });

  confirmDeleteButton = () => this.deleteModal().getByRole('button', { name: /^deactivate$/i });
  ...
  addButton = () => this.page.getByRole('button', { name: /Add Employee/i });
